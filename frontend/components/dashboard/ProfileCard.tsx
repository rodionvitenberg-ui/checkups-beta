...
import { 
    ...
    List, Activity, Loader2, Trash2, FileText 
} from 'lucide-react';
import { clsx } from 'clsx';
import { useToast } from '@/components/ui/toast';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { PatientChart } from '@/components/dashboard/PatientChart';
import { AnalysisItem } from './AnalysisItem'; // Импортируем наш вынесенный компонент
...
export function ProfileCard({ profile, isExpanded, onToggle }: { profile: PatientProfile, isExpanded: boolean, onToggle: () => void }) {
    ...
    // Проверяем, дефолтный ли это профиль (обычно называется "Анализы" или "Основной")
    ...
    const deleteProfileMutation = useMutation({
        ...
        onError: () => toast({ title: t('toasts.errorDeleteTitle'), description: t('toasts.errorDeleteDesc'), variant: "destructive" })
    });
